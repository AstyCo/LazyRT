...
const std::vector<std::string> Parser::_sourceFileExtensions = initSourceFileExtensions();
...
void Parser::parseDirectory(const char *directory_path)
{
    parseDirectory(BoostPath(directory_path));
}

void Parser::parseDirectory(const BoostPath &directory_path)
{
    _fileTree.clean();
...
void Parser::parseDirectoryRecursively(const BoostPath &directory_path, const BoostPath &dir_base)
{
    try {
...
void Parser::removeEmptyDirectories()
{
    MY_ASSERT(_fileTree._rootDirectoryNode);
    _fileTree.removeEmptyDirectories();
}

bool Parser::isSourceFile(const path &file_path) const
{
